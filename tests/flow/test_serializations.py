...
class test_v0_rdb_load:
    ...
    def test_v0_tf_model(self):
        ...
        self.env.assertEqual([backend, device, tag, batchsize, minbatchsize, inputs, outputs], [b"TF", b"CPU", b"TF_GRAPH_V0", 0, 0, [], []])
...
class test_v1_rdb_load:
    ...
    def test_v1_tf_model(self):
        ...
        con.restore(key_name, 0, model_rdb, True)
        _, backend, _, device, _, tag, _, batchsize, _, minbatchsize, _ , inputs, _, outputs = con.execute_command("AI.MODELGET", key_name, "META")
        self.env.assertEqual([backend, device, tag, batchsize, minbatchsize, inputs, outputs], [b"TF", b"CPU", b"TF_GRAPH_V1", 0, 0, [b"a", b"b"], [b"c"]])    
        tf_model_run(self.env, key_name)
